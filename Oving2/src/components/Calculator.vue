...
<div class="bad">
    <div class="calculator">
        <div class="number-pad">
            ...
            <div class="utilities">
                 <div class="utility button" @click="handleUtility(utility)" v-for="utility in utilities" :key="utility">{{utility}}</div>
            </div>
            <div class="numbers ">
                <div class="number button" @click="handleInput(10-index)" v-for="index in 10" :key="index">{{10-index}}</div>
                <div class="number button" @click="handleInput('.')">.</div>
            </div>
            <div class="operators ">
                <div class="operator button" @click="handleOperator(operator)" v-for="operator in operators" :key="operator">{{operator}}</div> 
            </div>
        </div>
    </div>
    <div class="logs">
    ...
    methods: {
        ...
        handleOperator(operator) {
            if(this.operator !== "") {
                ...
                //Logger regnestykket
                this.history.push(this.ongoing + " " + this.operator + " " + this.input + " = " + answer);
                
                if(operator === '=') {
                    this.ongoing = this.ongoing + " " + this.operator + " " + this.input + " =";
                    this.operator = "";
